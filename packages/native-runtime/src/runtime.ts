import { fail, NativeRuntimeError } from "./errors.js";
import { assertSerializable } from "./serializable.js";
import type { HostMessage, NativeRuntime, NativeRuntimeOptions, RuntimeMessage, Serializable, Unsubscribe } from "./types.js";

interface PendingRequest {
  readonly resolve: (value: Serializable | undefined) => void;
  readonly reject: (reason: NativeRuntimeError) => void;
}
...
export function createNativeRuntime(options: NativeRuntimeOptions): NativeRuntime {
  ...
  const runtime: NativeRuntime = {
    ...
    destroy(reason = "Native runtime destroyed") {
      if (!active) {
        return;
      }
      ...
      active = false;
      unsubscribeChannel();
      const error = new NativeRuntimeError(reason);
  ...
  function receiveResponse(message: HostMessage & { readonly kind: "response" }): void {
    ...
    request.reject(new NativeRuntimeError(message.error ?? "Native request failed"));
  }
...
function validateOptions(options: NativeRuntimeOptions): void {
  if (options === null || typeof options !== "object") {
    fail("Expected native runtime options");
  }
...
function assertActive(active: boolean): void {
  if (!active) {
    fail("Native runtime is destroyed");
  }
}
